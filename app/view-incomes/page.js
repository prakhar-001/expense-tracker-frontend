    ...
    <div className="flex flex-col items-center justify-between px-2 sm:px-12 pb-5 w-full h-max ">
      <div  className="flex justify-between flex-wrap w-full mb-5 text-base sm:text-2xl font-semibold">
        ...
        <div className="text-base flex justify-between gap-2 mx-0 w-full sm:w-max sm:mx-0 mt-2 sm:mt-0">
            ...
            <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className='rounded-xl px-2 border-2 border-blue-400 bg-white dark:border-slate-900'>
            ...
            <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className='rounded-xl px-2 border-2 border-blue-400 bg-white dark:border-slate-900'>
                {/* <option value="2024">2024</option> */}
                {Array.from({ length: 5 }, (_, i) => 2022 + i).map((year) => (
                 <option key={year} value={year}>{year}</option>
                ))}
            </select>
            <button onClick={() => setDetails(!details)} className="sm:hidden flex items-center gap-3 bg-blue-400 dark:bg-white p-1 px-3 rounded-xl"><FcViewDetails className="text-xl dark:text-black"/></button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between w-full h-full">
        ...
        <div className="pc-form sm:flex flex-col items-center justify-around border-2 w-full sm:w-1/3 bg-slate-200 dark:bg-gray-400 rounded-xl p-5 hidden h-[78.15vh]">        
            ...
            <form onSubmit={handleSubmitIncome} className="flex flex-col w-full px-5">
              ...
              <button type="submit" className="p-2 w-40 mx-auto rounded-xl bg-green-400 dark:bg-slate-800 dark:text-white mt-3 font-semibold">Add Income</button>            
          </form>
        </div>
        ...
        <div className="w-full sm:pl-5 overflow-x-scroll flex sm:hidden">
          <table className="table-auto w-full border h-full bg-slate-200 dark:bg-gray-400 p-10">
          {
            incomesData.length > 0 && (
            <thead className="w-full">
              <tr className="">
